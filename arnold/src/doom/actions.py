...
class ActionBuilder(object):
    ...
    def __init__(self, params):
        """
        Class to store the actions and provide ways to give custom actions for vizdoom.
        Args:
            params:
        """
        self.params = params
...
def create_action_set(action_combinations, use_continuous):
    ...
    An action is a combination of one or several buttons. The '+' merges buttons
    that should not be triggered together. The ';' separates groups of buttons
    that can be triggered simultaneously. For example: Input:

        'turn_lr+move_fb;move_lr'

    Output:
        [['MOVE_LEFT'],
            ['MOVE_RIGHT'], ['TURN_LEFT'], ['TURN_LEFT', 'MOVE_LEFT'],
            ['TURN_LEFT', 'MOVE_RIGHT'], ['TURN_RIGHT'], ['TURN_RIGHT',
            'MOVE_LEFT'], ['TURN_RIGHT', 'MOVE_RIGHT'], ['MOVE_FORWARD'],
            ['MOVE_FORWARD', 'MOVE_LEFT'], ['MOVE_FORWARD', 'MOVE_RIGHT'],
            ['MOVE_BACKWARD'], ['MOVE_BACKWARD', 'MOVE_LEFT'], ['MOVE_BACKWARD',
            'MOVE_RIGHT']]
    ...
    Args:
        action_combinations: String giving the set of possible actions in the game scenario.
        use_continuous: Flag for using the actions in a continous space.
    """
    if use_continuous:
        ...
        # print( 'Actions-- ',sum([action_categories_discrete[y] for y in x.split('+')], [None] ) )
        action_subsets = [
