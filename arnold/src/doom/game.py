...
class Game(object):
    ...
    def __init__(
        ...
        """Create a new game. score_variable: indicates in which game variable
        the user score is

            stored. by default it's in FRAGCOUNT, but the score in ACS against
            built-in AI bots can be stored in USER1, USER2, etc.

        render_decals: marks on the walls render_particles: particles like
        for impacts / traces render_effects_sprites: gun puffs / blood splats
    ...
    def update_game_variables(self):
        ...
        assert all(v.is_integer() or k[-2:] in ['_x', '_y', '_z'] for k, v in new_v.items())   ## except postions(floats) all other game variables are ints.
    ...
    def update_statistics_and_reward(self, action):
        """Update statistics of the current game based on the previous and the
        current properties, and create a reward.
    ...
    def log(self, message):
        """Log the game event. During training, we don't want to display events.
    ...
    def start(self, map_id, episode_time=None, manual_control=False, log_events=False):
        """Start the game. If `episode_time` is given, the game will end after
        the specified time.
    ...
    def update_bots(self):
        """Add built-in AI bots. There are two types of AI: built-in AI and
        ScriptedMarines.
        """
    ...
    def is_episode_finished(self):
        """Return whether the episode is finished. This should only be the case
        after the episode timeout.
        """
    ...
    def initialize_game(self):
        """Initialize the game after the player spawns / respawns. Be sure that
        properties from the previous life are not considered in this one.
        """
        # generate buffers
    ...
    def make_action(self, action, frame_skip=1, sleep=None):
        """Make an action. If `sleep` is given, the network will wait `sleep`
        seconds between each action.
    ...
    def print_statistics(self, eval_time=None):
        """Print agent statistics. If `map_id` is given, statistics are given
        for the specified map only. Otherwise, statistics are given for all
        maps, with a summary.
